...
// AddTargetRequestAndReferer adds one new Request waitting for crawl.
func (this *Page) AddTargetRequestAndReferer(url string, respType string, referer string) *Page {
	this.targetRequests = append(this.targetRequests, request.NewRequestWithReferer(url, respType, "", "GET", "", nil, nil, referer, nil, nil))
	return this
}

// AddTargetRequestsAndReferer adds new Requests waitting for crawl.
func (this *Page) AddTargetRequestsAndReferer(urls []string, respType string, referer string) *Page {
	for _, url := range urls {
		this.AddTargetRequestAndReferer(url, respType, referer)
	}
	return this
}
